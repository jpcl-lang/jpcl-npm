...
├─ core.ts       loads/dumps and types, no filesystem -- the "jpcl/core" export
...
The package mirrors [jpcl-py](https://github.com/jpcl-lang/jpcl-py) module for
...
`jpcl/core` keeps working in browsers.
...
`jpcl fmt` is deliberately *not* enforced in CI: rewriting a file drops its
...
2. npm only lets you configure a trusted publisher for a package that already
   ...
   ```
3. On <https://www.npmjs.com/package/jpcl/access>, add a trusted publisher:

   | Field | Value |
   | --- | --- |
   | Publisher | GitHub Actions |
   | Organization or user | `jpcl-lang` |
   | Repository | `jpcl-npm` |
...
#    the exported `version` and `jpcl --version` read it at runtime.
